...
IMPORTANT: Return ONLY valid JSON without any extra text, explanations, or markdown formatting. If any field is missing, use an empty string or array.
    `;
    ...
    const completion = await groq.chat.completions.create({
      messages: [
        {
          role: 'system',
          content: 'You are a resume parsing assistant. Extract information from resumes and return ONLY valid JSON with no additional text or formatting.'
        },
        {
          role: 'user',
          content: prompt
...
Resume Data: ${JSON.stringify(resumeData)}`;
    ...
    const completion = await groq.chat.completions.create({
      messages: [
        {
          role: 'system',
          content: 'You are an ATS resume scorer. Return only a number between 0-100.'
        },
        {
          role: 'user',
          content: scorePrompt
